...
class BertModel:
    ...
    def fit(self, ):
        """
        Using loaded by "load_data" method datadets, finetune BERT models and save them to data/ directory
        :return:
        """
    ...
    def get_embeddings(self, input_f='data/train_only_text.pkl', output_f='data/train_bert_7.pkl'):
        """
        Encode texts from predefined datasets and save results to file
        :param input_f: string, path to file with dataset to encode
        :param output_f: strin, path to encoded dataset
        :return:
        """
        ...
        tmp = self._denormalize_encodings(train)
    ...
    def load_models(self, path=DATAPATH, model_1='model_dump2020-06-29_18-24-52',
        ...
        :param model_1: string, name of directory for first level model
        :param model_2: string, name of directory for second level model
        :return:
        """
        model_save_path = path + model_1
        model_save_path2 = path + model_2
        self.model1 = SentenceTransformer(model_save_path)
        self.model2 = SentenceTransformer(model_save_path2)

    def model_encode(self, text, af, rf, ap, rp):
        """
        Having basic input of the general model encode it using BERT model for further usage
        :param text: string
        :param af: string
        :param rf: string
        :param ap: string
        ...
        train["af_embed"] = self.model2.encode([af])
        train["rf_embed"] = self.model2.encode([rf])
        train["ap_embed"] = self.model1.encode([ap])
        train["rp_embed"] = self.model1.encode([rp])

        train["text_embed"] = self.model1.encode([text])
        train["text_embed_2"] = self.model2.encode([text])
    ...
    @staticmethod
    def _denormalize_encodings(train):
        ss = train["ap_embed"][0].shape[0]
